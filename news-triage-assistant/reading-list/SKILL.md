...
- **Bot seeds the reactions, the user's click is the signal.** `fetch_message_reactions` in `lib/discord.py` computes `by_user` as `count > (1 if me else 0)` — the bot's own seed reaction must stay at exactly one, so never react twice from the bot.
